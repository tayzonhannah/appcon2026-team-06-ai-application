    ...
    <div className="mx-auto w-full max-w-3xl py-2 sm:py-10">
      ...
      {earned > 0 ? (
        <div className="mt-4 rounded-[20px] border-4 border-[#0F172A] bg-[#B7E4C7] p-5 shadow-[0_5px_0_#0F172A]" aria-live="polite">
          ...
          <p className="mt-1 text-sm font-bold text-[#0F172A]/70">Added to your token balance. Tokens last 24 hours.</p>
        </div>
      ) : null}

      <div className="mt-4 rounded-[24px] border-4 border-[#0F172A] bg-[#0F172A] p-6 text-white shadow-[0_7px_0_#0F172A] sm:p-8">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-[#DBEAFE]">My tokens</p>
      ...
      <h2 className="mt-8 text-xl font-black">Token history</h2>
      {all.length === 0 ? (
        <div className="mt-3 rounded-[20px] border-4 border-dashed border-[#0F172A]/30 bg-white/70 p-6 text-sm font-bold text-[#475569]">
          No tokens yet. Your earned tokens will show up here with expiry times.
        </div>
      ) : (
        <ul className="mt-3 flex flex-col gap-3">
          {all.map((token) => {
            const expired = new Date(token.expiresAt).getTime() <= now;
            const minsLeft = Math.max(0, Math.ceil((new Date(token.expiresAt).getTime() - now) / 60000));
            return (
              <li key={token.id} className={`rounded-[18px] border-4 border-[#0F172A] p-4 shadow-[0_4px_0_#0F172A] ${expired ? "bg-white/60 opacity-70" : "bg-white"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black">{tokenLabel(token.actionId)}</p>
                    <p className="mt-1 text-xs font-bold text-[#475569]">
                      {expired ? "Expired" : `Expires in ${Math.floor(minsLeft / 60)}h ${minsLeft % 60}m`}
                    </p>
                  </div>
